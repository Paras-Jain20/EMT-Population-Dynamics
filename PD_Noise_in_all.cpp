...
typedef boost::array<double, NUMNODES> cell_state; // for cells' variable vector for ODE simulation function
typedef boost::unordered_map<int, boost::array<double, NUMNODES>> population; // stores the values of 4 variables for every cell

std::mt19937 generator; // for random number generation

double temp_var[11]; // temp_var hold the value for initialization parameters provided at start 
...
////  Based on cell's position in attractor basin of an phenotype as per mZEB - SNAIL bifurcation plot ///
...
void initialize_Signal_lognormal(population& P, double* pop_fraction, const int pop_size)
{
...
// to load eigen values matrix (save steady_states.csv in working directory) ////
	...
	{
		I = std::exp(std::log(M) + dist(generator) * SD); // random selection of a SNAIL value
...
void add_sym_noise_normal(population &P, int rep_id, int new_id, double * eta, int eta_id)
{
	...
	for (int i = 0; i < NUMNODES; i++){
    	...
    	P[rep_id][i] = P[rep_id][i] + rand_num[0]*eta[0]*P[rep_id][i]/2 + rand_num[1]*eta[1]*(2*P[rep_id][i] + rand_num[0]*eta[0]*P[rep_id][i]) ;
        P[new_id][i] = P[rep_id][i] + rand_num[0]*eta[0]*P[rep_id][i]/2 - rand_num[1]*eta[1]*(2*P[rep_id][i] + rand_num[0]*eta[0]*P[rep_id][i]) ;
...
////////// sampling a fraction of cell's subpopulation when overall population size increase 80% of carrying capacity K
...
////////////////////////// population dynamics ////////////////////////////////////////////////////////////
int simulate_normal(population& P, double end_time, double * eta, int eta_id,int frac_id, int sim_type, boost::array <double, 3> GR, int GR_ratio_id, int pop_size, int sim_num)
{
	...
	if(sim_type == 1){
		...
		// Below, 'E_frac_spread_Data_20hrs_DT_32_weeks' is the folder name in which output data files will be stored.
		// Create it in current working directory before running the simulation. 
		end_of_day.open("Data_20hrs_DT_32_weeks/end_of_day_counts_" + std::to_string(eta_id) + "_" + std::to_string(frac_id) + "_" + std::to_string(GR_ratio_id) + ".csv", std::ios::app);
		
		SNAIL_distribution.open("SNAIL_dist_20hrs_DT_32_weeks/snail_dist_" + std::to_string(eta_id) + "_" + std::to_string(frac_id) + "_" + std::to_string(GR_ratio_id) + "_" + std::to_string(sim_num) + ".csv");

		mZEB_distribution.open("ZEB_dist_20hrs_DT_32_weeks/mZEB_dist_" + std::to_string(eta_id) + "_" + std::to_string(frac_id) + "_" + std::to_string(GR_ratio_id) + "_" + std::to_string(sim_num) + ".csv");
	...
	while(t <= end_time)
	{
		...
		if(sim_type == 1)
		{
			...
			if (t < 24* day_indx) {
				...
				if((int(day_indx)%14) == 0){ // save SNAIL and ZEB levels of cells every two weeks
					for(int i = 0; i < P.size(); i++){
						if(i < P.size() -1){
							SNAIL_distribution << P[i][3] << ",";
							mZEB_distribution << P[i][1] << ",";
...
// e.g. 1_0.2_0.1_0_1_0_0_0_20_20_20_0_50000
	...
	int sim_num = 1;
	int pop_size = 200;
	int total_inde_runs = 16;
	int time_in_days = 32 * 7 ; // 32 respresent 32 weeks
	int file_rows = 0;
	...
	while(true){
		...
		csv_file.open("Data_20hrs_DT_32_weeks/end_of_day_counts_" + std::to_string(eta_id) + "_" + std::to_string(frac_id) + "_" + 
			std::to_string(GR_ratio_id) + ".csv");
		...
		initialize_Signal_lognormal(P, pop_fraction, 200); // change initial pop size (200 now) as 1 for single cell simualtion
		...
		std::cout << "initialization complete for eta id " << eta_id << " frac_id " << frac_id << " GR ratio id " << GR_ratio_id <<" and sim num is " << sim_num << std::endl;

		aborted = simulate_normal(P, 24 * time_in_days, eta, eta_id, frac_id, 1, GR, GR_ratio_id, pop_size, sim_num);
		...
		if(aborted == 1)
		{
			...
			std::cout << "simulation complete for eta id " << eta_id << " frac_id " << frac_id << " GR ratio id " << GR_ratio_id <<" and sim num is " << sim_num << std::endl;
			sim_num++;
		}
